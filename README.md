...
Undo/redo also have toolbar buttons (next to "Clear console"), disabled when there's
nothing to undo/redo. They cover every command-driven change - typed or from a canvas
gesture, since those synthesize the same commands - but not raw node-position dragging,
which never runs through the command log. The history is per-tab and not persisted: it
resets on reload and is cleared by "Clear history" or a state update arriving from another
tab, so it never points at a snapshot the current architecture didn't come from.
...
- `undo`/`redo` are two stacks of `{ command, snapshot }` pairs (`lib/undo-history.ts`, pure and unit-tested on its own), pushed to by every successful architecture-mutating command; a fresh command clears the redo branch, the standard rule. The history is intentionally ephemeral rather than persisted - full architecture snapshots for a long session's worth of commands risks real `localStorage` bloat, and it's cleared on hydration, a cross-tab sync, or "Clear history" so it never restores a snapshot the current architecture didn't actually come from.
