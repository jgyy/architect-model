...
export const SUPPORTED_COMMANDS = [
    ...
    'connect <A> to <B>            — e.g. "connect Web Server to Cache" (aliases: connect A and B, link A to B, link A and B) — A and B may be the same node (a self-loop), and cycles across nodes are allowed',
    ...
    'rename node <A> to <B>        — e.g. "rename node Cache to Redis" (alias: relabel node A to B)',
];
