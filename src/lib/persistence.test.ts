...
const sampleState: PersistedState = {
  ...
  log: [{ id: 1, input: "add node Cache", ok: true, message: 'Added node "Cache".' }],
};
...
describe("loadPersistedState", () => {
  ...
  test("returns null when the stored value doesn't look like a PersistedState", () => {
    const storage = createFakeStorage();
    storage.setItem("architect-model:session", JSON.stringify({ foo: "bar" }));
  ...
  test("returns null when a persisted log entry is missing a numeric id", () => {
    ...
    storage.setItem(
      ...
      JSON.stringify({
        ...
        log: [{ input: "add node Cache", ok: true, message: 'Added node "Cache".' }],
      }),
    );
